...
class PenindakanController extends Controller
{
    public function createOrUpdate(Request $request)
    {
            ...
            $penindakan = Penindakan::where('id_keluhan', $request->id_keluhan)->first();
            ...
            if ($penindakan) {
                ...
                return redirect()->route('keluhan.show', ['keluhan' => $keluhan, 'penindakan' => $penindakan])->with(['status' => 'Berhasil Dikirim!']);
        } else {
            $keluhan->update(['status' => $request->status]);
